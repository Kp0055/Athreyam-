...
// Chat
import { getUserConversations } from "../../controller/chat/chatController";
// Reusing findDoctor for chat search. If needed we can create a specific searchDoctors controller that supports query params.
// The existing findDoctor fetches ALL, we might want to filter on frontend or update it.
// For now, let's keep it simple as user requested only UI.
router.get("/users/conversations", authMiddleware, getUserConversations);

// Wallet
import getWalletHistory from '../../controller/user/getWalletHistory';
router.get("/users/wallet", authMiddleware, getWalletHistory);
